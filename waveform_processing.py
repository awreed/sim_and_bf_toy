...
def delay_waveforms(RP, ps, amplitudes, noise=False, noise_std=.01, temps=None,
    min_dist=0.0, single=False, phase_shift=np.array([0.]), scat_phase=None):
    ...
    #if scat_phase:
    #  scat_p = np.random.uniform(0, 2*np.pi,
    #      size=amplitudes_stacked.detach().cpu().numpy().shape)
    #  scat_p = torch.from_numpy(scat_p)
    #else:
    #  scat_p = torch.zeros_like(amplitudes_stacked).to(amplitudes_stacked.device)
